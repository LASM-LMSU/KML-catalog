...
test("карточка выбранного слоя сворачивается и разворачивается", async ({ page }) => {
  await page.goto("/");
  ...
  const selection = page.locator(".selection-context");
  await expect(selection).toBeVisible();
  await expect(selection.locator(".selection-actions")).toHaveCount(1);

  await page.getByRole("button", { name: "Свернуть карточку выбранного слоя" }).click();
  await expect(selection).toHaveClass(/is-collapsed/);
  await expect(selection.locator(".selection-actions")).toHaveCount(0);

  await page.getByRole("button", { name: "Развернуть карточку выбранного слоя" }).click();
  await expect(selection).not.toHaveClass(/is-collapsed/);
  await expect(selection.locator(".selection-actions")).toHaveCount(1);
});
...
test("таймлайн сворачивается до компактной линии и разворачивается обратно", async ({ page }) => {
  ...
  if (!wasCollapsedInitially) {
    await page.getByRole("button", { name: "Свернуть таймлайн" }).click({ force: true });
  }
  ...
  const expandButton = page.getByRole("button", { name: "Развернуть таймлайн" });
  ...
  if (collapsedPanelBox && expandBox) {
    expect(expandBox.width).toBeGreaterThanOrEqual(70);
    expect(expandBox.height).toBeGreaterThanOrEqual(36);
    expect(expandBox.x - collapsedPanelBox.x).toBeLessThanOrEqual(32);
  }
